...
void printField(WINDOW *w, Tgame *game) {
  box(w, 0, 0);  // рисуем рамку
  for (int i = 0; i < game->rows; i++) {
    wmove(w, 1 + i, 1);  // сдвигаем курсор ниже рамки
  ...
  wnoutrefresh(w);  // копирует содержимое окна w во внутренний буфер обновлений
                    // ncurses, но не обновляет экран непосредственно
}

void printNext(WINDOW *w, Tgame *game) {
  wclear(w);
  box(w, 0, 0);
  if (game->next->type == -1) {  // существует ли следующая фигура, -1 - нет
    wnoutrefresh(w);
    return;
  }
